...
case "${ARCH}" in
	"amd64") ARCH_REPO="CODE-ubuntu1804";; # untested
	"arm64") ARCH_REPO="CODE-arm64-ubuntu1804";;
esac
...
src_install() {
	...
	fowners cool:cool /etc/coolwsd/coolwsd.xml /opt/cool/child-roots
	fperms 640 /etc/coolwsd/coolwsd.xml
	fperms 750 /opt/cool/child-roots
...
pkg_postinst() {
	...
	einfo "Example configuration files for Apache and nginx have been installed to:"
	einfo "	/usr/share/doc/${P}/examples"
	einfo "After every install of upgrade, you are advised to update the system template:"
	einfo "	coolwsd-systemplate-setup /opt/cool/systemplate /opt/collaboraoffice"
	einfo "and generate the proof keys:"
	einfo "	coolwsd-generate-proof-key"
	einfo "Or, alternatively, run the config function of this package:"
